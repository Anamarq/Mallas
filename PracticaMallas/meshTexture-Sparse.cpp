...
int main (int argc, char *argv[])
{
	...
	try
	{
		...
		Eigen::SparseMatrix<double, Eigen::RowMajor>meshMatrix(numVertex, numVertex);
		...
		//			for (int k = 0; k < neighbour[k].size(); ++k) {
		...
		//				}
		//			}
		//			tripleList.push_back(T(i, j, solCot));
		//			solCot = 0; //reiniciar a 0 para el siguiente vértice/posición de la matriz
		//		}			
		//	}
		//}
		//meshMatrix.setFromTriplets(tripleList.begin(), tripleList.end());
		...
		for (int i = 0; i < numVertex; ++i) {
			for (int j = 0; j < numVertex; ++j) {
				...
				if (std::find(neighbour[i].begin(), neighbour[i].end(), j) != neighbour[i].end())
				{
					...
					meshMatrix.coeffRef(i, j) = solCot;
					solCot = 0; //reiniciar a 0 para el siguiente vértice/posición de la matriz
				}
		...
		/*for (int k = 0; k < meshMatrix.outerSize(); ++k){
			for (SparseMatrix<double, Eigen::RowMajor>::InnerIterator it(meshMatrix, k); it; ++it) {
				if (it.row() == it.col())
					it.valueRef()= -meshMatrix.row(it.row()).sum();
		...
		for (int i = 0; i < numVertex; ++i)
			meshMatrix.coeffRef(i, i) = -meshMatrix.row(i).sum();
		//muestra matriz por pantalla (borrar)
		...
		//std::set<int>externalAux(size_t(numOfExternalVertex));
		//externalAux.insert(externalAux.end(), externalVertex.begin(), externalVertex.end());
		for (int i = 0; i < numVertex; ++i) {
			for (int j = 0; j < numVertex; ++j) {
				if (std::find(externalVertex.begin(), externalVertex.end(), i) != externalVertex.end())
				{
					if (i != j)
						meshMatrix.coeffRef(i, j) = 0;
					else
						meshMatrix.coeffRef(i, j) = 1;
				}
			}
		}
        //END TODO 3.2
		...
		if (solver.info() != Eigen::Success)
			throw "solver.compute() failed";

		UV = solver.solve(UV_0);
		if (solver.info() != Eigen::Success)
			throw "solver.solve() failed";
